...
def figure1():
    ...
    rows=[
        ("Query\nexecution",
         ...
         "broad-market benchmark over the post-report window?  (Indonesia / IDX, FY2025)", 15),
        ("Result entity",
        ...
        ("Deciding\nobservations",
         ...
         r"$\Rightarrow$  −0.74% > −1.61%  and  −0.74% > −9.48%  :  outperforms sector and benchmark", 18),
    ...
    gap=3.0; top=90.0; lx=1.0; lw_=19.0; cx=22.0; cw=77.0
    ...
    for label,content,h in rows:
        ...
        ax.text(cx+2.2,y-h/2,content,ha="left",va="center",color=INK,fontsize=8.8)
...
# ---------------- Figure 2 ----------------
def figure2():
    fig,ax=plt.subplots(figsize=(8.2,5.0)); ax.axis("off"); ax.set_xlim(0,100); ax.set_ylim(0,100)
    layers=[
        ("L0","Source","Per-market sheets: scope, company master, fundamentals, market windows,\n"
         "comparators, FX, announcements, provenance, query cases", ORANGE, ORANGE_EDGE, True),
        ("L1","Materialization","Provenance-aware RDF aligned to the core ontology", CORE_BLUES[0], EDGE, False),
        ("L2","Derived metrics","window / sector / benchmark return, YoY growth, FX association,\n"
         "event-window abnormal return, event-window abnormal volume", CORE_BLUES[1], EDGE, False),
        ("L3","SHACL validation","gate: Conforms = True", CORE_BLUES[2], EDGE, False),
        ("L4","CQ answering","CQ1–CQ5 SPARQL templates (compute each stated condition)", CORE_BLUES[3], EDGE, False),
        ("L5","Inference","rules R1–R4 → typed AnalyticalFindings", CORE_BLUES[4], EDGE, False),
        ("L6","Automated explanation","EvidenceBundle + QueryExecution (the answer's evidence trail)", CORE_BLUES[5], EDGE, False),
    ]
    ...
    for i,m in enumerate(["PSX","MSX","IDX"]):
        ...
        ax.text(bx+5.0,93.6,m,ha="center",va="center",color="white",fontweight="bold",fontsize=9)
    ...
    for code,title,desc,fc,ec,dep in layers:
        ...
        ax.text(cx+1.6,y-h/2,code,ha="left",va="center",fontweight="bold",fontsize=11,color=NAVY)
        ax.text(cx+8.5,y-h*0.34,title,ha="left",va="center",fontweight="bold",fontsize=9.6,color=INK)
        ax.text(cx+8.5,y-h*0.72,desc,ha="left",va="center",fontsize=7.8,color="#33414d")
        ys.append((y,y-h)); y=y-h-gap
    rbox(ax,cx+cw+1.4,top-h,9.2,h,ORANGE,ORANGE_EDGE,lw=0.9)
    ax.text(cx+cw+6.0,top-h/2,"adapter\nconfig\n(~9 keys)",ha="center",va="center",fontsize=7.4,color=INK)
    ...
    ax.text(cx,top+1.4,"market-dependent",ha="left",va="bottom",fontsize=7.6,fontweight="bold",color="#a8631e")
    ...
    for j,name in enumerate(core):
        ...
        ax.text(bx+bw/2,yy-bh/2,name,ha="center",va="center",fontsize=7.8,color=INK)
    ...
    ax.text(5.5,(band_top+band_bot)/2,"market-independent core\nreused byte-identical · 13 artifacts (100%)",
            ha="center",va="center",rotation=90,fontsize=7.8,fontweight="bold",color=NAVY)
    rbox(ax,91.4,ys[-1][1]-0.4,7.2,top-(ys[-1][1]-0.4),"#fbeede",ORANGE_EDGE,lw=0.9)
    ax.text(95.0,(top+ys[-1][1])/2,"market-dependent\nonly data + adapter",
            ha="center",va="center",rotation=90,fontsize=7.8,fontweight="bold",color="#a8631e")
    fig.tight_layout(pad=0.2)
